...
export default function UACSuper() {
  ...
  const navLinks = [
    ...
    { to: "/UACSuper", img: IconUACBlue, label: "User Access Control" },
  ...
  const RoleBadge = ({ value }) => {
    ...
    return (
      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${getColorClasses(value)}`}>
        {value}
      </span>
    );
  };
  ...
  const saveEdits = async () => {
    ...
    setSavingEdit(true);
    try {
      await setDoc(doc(db, "users", viewing.id), {
        email: edit.email,
        role: edit.role,
          ...
          >
            <div className="relative flex items-center justify-between px-6 py-4 border-b bg-white/70 backdrop-blur">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-semibold text-gray-800">Manage User Access</h3>
