...
export default function VehicleManagement() {
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredWithRowNumber.map((item) => [
      ...
      item.serialNo,
      item.fuel,
      item.routeName,  // Displaying Route Name instead of Route ID
      item.status,
    ]);
    const data = [headers, ...rows];
    const csv = Papa.unparse(data);
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    saveAs(blob, "vehicle-management-report.csv");
  };
    ...
    sections: [
      {
        ...
        children: [
          new Paragraph({
            children: [
              new TextRun({
                text: "Vehicle Management Report",
                bold: true,
                size: 48,
                font: "Arial",
                color: "364C6E",
  ...
  Packer.toBlob(doc)
    .then((blob) => {
      saveAs(blob, "vehicle-management-report.docx");
    })
    .catch((error) => {
  ...
  const exportToPDF = () => {
    ...
    doc.text("Vehicle Management Report", 20, 20);
  ...
  const unsub = onSnapshot(collection(db, "vehicle"), (snap) => {
    try {
      const temp = [];
      snap.forEach((d) => {
        const data = d.data();
  ...
  const routeUnsub = onSnapshot(collection(db, "routes"), (snap) => {
    try {
      const temp = [];
      snap.forEach((d) => {
        const data = d.data();
        if (data && data.Route) {  // Note: "Route" with capital R
          temp.push({
            id: d.id,
  ...
  const StatusBadge = ({ value }) => {
    ...
    return (
      ...
      >
        <span className={`mr-1.5 inline-block h-1.5 w-1.5 rounded-full ${isActive ? "bg-green-500" : "bg-gray-400"}`} />
  ...
  const columns = [
    {
      ...
      sortable: true,
      cell: (r) => <div className="truncate" title={r.vehicleID}>{r.vehicleID}</div>,
    },
    {
      name: "Unit",
      selector: (r) => r.unit,
      sortable: true,
      cell: (r) => <div className="truncate" title={r.unit}>{r.unit}</div>,
    },
    {
      name: "Serial No",
      selector: (r) => r.serialNo,
      sortable: true,
      cell: (r) => <div className="truncate" title={r.serialNo}>{r.serialNo}</div>,
    },
    {
      name: "Fuel",
      selector: (r) => r.fuel,
      sortable: true,
      cell: (r) => <div className="truncate" title={r.fuel}>{r.fuel}</div>,
    },
        {
      name: "Route",
      selector: (r) => {
            ...
            <div className="relative flex items-center justify-between px-6 py-4 border-b bg-white/70 backdrop-blur">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{viewing.vehicleID}</p>
                </div>
              </div>
            ...
            <div className="p-10 grid ml-6 grid-cols-3 gap-x-5 gap-y-4">
              <div className="col-span-1">
                ...
                <input
                  ...
                  onChange={(e) => setEdit({ ...edit, vehicleID: e.target.value })}
                />
              </div>
              <div className="col-span-1">
              ...
              <div className="col-span-1">
                ...
                <select
                  value={edit.routeId}
                  onChange={(e) => setEdit({ ...edit, routeId: e.target.value })}
                  className="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-300"
                >
                  {routes.map((route) => (
                    <option key={route.id} value={route.id}>
